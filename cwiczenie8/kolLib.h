...
mqd_t stworzMQ(const char* nazwa, struct mq_attr *attr) {
    ...
    mqd_t des = open(nazwa, O_CREAT | O_EXCL | O_RDWR, 0666, NULL);
    if(des == -1) {
        perror("ERROR: open - stworzMQ - kolLib.h");
        exit(1);
    }
...
mqd_t otworzMQ_Read(const char* nazwa) {
    ...
    mqd_t des = open(nazwa, O_RDONLY);
    if(des == -1) {
        perror("ERROR: open - otworzMQ_Read - kolLib.h");
        exit(1);
    }
...
mqd_t otworzMQ_Write(const char* nazwa) {
    ...
    mqd_t des = open(nazwa, O_WRONLY);
    if(des == -1) {
        perror("ERROR: open - otworzMQ_Write - kolLib.h");
        exit(1);
    }
